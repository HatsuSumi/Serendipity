...
const payment_dto_1 = require("../../../src/types/payment.dto");
describe('PaymentService', () => {
    let paymentService;
    let mockPaymentOrderRepository;
    ...
    beforeEach(() => {
        ...
        paymentService = new paymentService_1.PaymentService(mockPaymentOrderRepository, mockMembershipRepository, mockLogger);
    });
    describe('createPayment', () => {
        it('应该创建免费支付（¥0，不创建订单）', async () => {
            const userId = 'user123';
            const data = { amount: 0, method: payment_dto_1.PaymentMethod.FREE };
            ...
            expect(result.method).toBe(payment_dto_1.PaymentMethod.FREE);
            expect(result.status).toBe(payment_dto_1.PaymentStatus.SUCCESS);
        ...
        it('应该创建 Mock 支付订单（微信支付）', async () => {
            const userId = 'user123';
            const data = { amount: 100, method: payment_dto_1.PaymentMethod.WECHAT };
            mockPaymentOrderRepository.create.mockResolvedValue({
                id: 'ORDER_123',
                userId,
                amount: 100,
                method: payment_dto_1.PaymentMethod.WECHAT,
                status: payment_dto_1.PaymentStatus.PENDING,
            });
            const result = await paymentService.createPayment(userId, data);
            expect(result.amount).toBe(100);
            expect(result.method).toBe(payment_dto_1.PaymentMethod.WECHAT);
            expect(result.status).toBe(payment_dto_1.PaymentStatus.PENDING);
        ...
        it('应该拒绝超出范围的金额', async () => {
            const userId = 'user123';
            const data = { amount: 1000, method: payment_dto_1.PaymentMethod.WECHAT };
            await expect(paymentService.createPayment(userId, data)).rejects.toThrow('Payment amount must be between ¥0 and ¥648');
        });
        it('应该拒绝负数金额', async () => {
            const userId = 'user123';
            const data = { amount: -10, method: payment_dto_1.PaymentMethod.WECHAT };
            await expect(paymentService.createPayment(userId, data)).rejects.toThrow('Payment amount must be between ¥0 and ¥648');
        });
    });
    ...
    describe('getMembershipStatus', () => {
        it('应该返回现有会员状态', async () => {
            ...
            const mockMembership = {
                userId,
                tier: payment_dto_1.MembershipTier.PREMIUM,
                status: payment_dto_1.MembershipStatus.ACTIVE,
                startedAt: new Date(),
                expiresAt: new Date(),
                autoRenew: false,
                monthlyAmount: 100,
            ...
            expect(result.tier).toBe(payment_dto_1.MembershipTier.PREMIUM);
            expect(result.status).toBe(payment_dto_1.MembershipStatus.ACTIVE);
        ...
        it('应该为新用户创建免费会员记录', async () => {
            ...
            mockMembershipRepository.create.mockResolvedValue({
                userId,
                tier: payment_dto_1.MembershipTier.FREE,
                status: payment_dto_1.MembershipStatus.INACTIVE,
                autoRenew: false,
            });
            const result = await paymentService.getMembershipStatus(userId);
            expect(result.tier).toBe(payment_dto_1.MembershipTier.FREE);
            expect(result.status).toBe(payment_dto_1.MembershipStatus.INACTIVE);
    ...
    describe('handleWechatCallback', () => {
        it('应该处理微信支付回调', async () => {
            ...
            expect(mockPaymentOrderRepository.updateStatus).toHaveBeenCalledWith('ORDER_123', payment_dto_1.PaymentStatus.SUCCESS, 'TXN_123', callbackData.paidAt);
        ...
        it('应该忽略已处理的订单', async () => {
            ...
            const mockOrder = {
                ...
                amount: 100,
                method: payment_dto_1.PaymentMethod.WECHAT,
                status: payment_dto_1.PaymentStatus.SUCCESS, // 已经成功
            };
            mockPaymentOrderRepository.findById.mockResolvedValue(mockOrder);
            await paymentService.handleWechatCallback(callbackData);
    ...
    describe('handleAlipayCallback', () => {
        it('应该处理支付宝回调', async () => {
            ...
            const mockOrder = {
                ...
                userId: 'user123',
                amount: 100,
                method: payment_dto_1.PaymentMethod.ALIPAY,
                status: payment_dto_1.PaymentStatus.PENDING,
            };
            mockPaymentOrderRepository.findById.mockResolvedValue(mockOrder);
            mockPaymentOrderRepository.updateStatus.mockResolvedValue({});
            ...
            expect(mockPaymentOrderRepository.updateStatus).toHaveBeenCalledWith('ORDER_123', payment_dto_1.PaymentStatus.SUCCESS, 'ALIPAY_TXN_123', callbackData.paidAt);
            expect(mockMembershipRepository.activateOrCreate).toHaveBeenCalled();
        });
    });
});
