...
Future<void> createTestRecords() async {
  ...
  final deviceIdentityService = DeviceIdentityService(storage: storageService);
  ...
  try {
    ...
    final authRepo = CustomServerAuthRepository(
    ...
    try {
      ...
      for (int i = 0; i < 500; i++) {
        ...
        final conversationStarter =
            status == EncounterStatus.met && random.nextDouble() < 0.5
                ? sampleConversationStarter[
                    random.nextInt(sampleConversationStarter.length)
                  ]
                : null;
        ...
        final record = EncounterRecord(
          id: uuid.v4(),
          timestamp: timestamp,
          anniversaryMonth: timestamp.month,
          anniversaryDay: timestamp.day,
          location: location,
      ...
      for (int i = 0; i < allRecords.length; i += batchSize) {
        final batch = allRecords.sublist(
          i,
          (i + batchSize).clamp(0, allRecords.length),
        );
        await remoteRepo.uploadRecords(result.user.id, batch);
        print(
          '☁️ 已上传 ${(i + batch.length).clamp(0, allRecords.length)}/${allRecords.length} 条记录',
        );
      }
      print('✅ 服务端上传完成！');
