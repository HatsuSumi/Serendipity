...
const List<AboutSectionContent> _staticAboutPageSections = [
  ...
  AboutSectionContent(
    title: '项目说明',
    paragraphs: [
      '这是一个由一人耗时71天，在孤独和抑郁的陪伴下完成开发的开源免费项目。',
...
final List<AboutSectionContent> designDecisionSections = List.unmodifiable([
  ...
  const AboutSectionContent(
    title: '为什么签到提醒有时候没收到？',
    paragraphs: [
      '先说结论：当前签到提醒主要依赖服务端远程推送，使用的是 FCM（Firebase Cloud Messaging），所以如果你偶尔收不到，常见原因不一定是你没设置时间，而更可能是通知权限、系统限制，或者当前网络环境下 FCM 链路不稳定。',
      'Serendipity 不是单纯依赖本地定时器在手机里响铃，而是主要由服务端在合适的时间发出推送，再由 FCM 把消息送到你的设备。\n\n这意味着：只要推送链路里的任意一环出问题，你都可能收不到提醒。',
      '为什么有时候会“没收到”？\n\n常见原因包括：\n1. 没有授予通知权限\n2. 系统电池优化、后台限制、自启动限制影响了消息接收\n3. app 被系统冻结、清理，或者长期不活跃\n4. 当前网络环境下 FCM 连接不稳定\n5. 某些地区、网络或设备环境下，可能需要开启 VPN，FCM 才更容易正常工作',
      '如果你所在的网络环境对 Google/Firebase 相关服务访问不稳定，那么即使 app、本机设置、服务端都没问题，推送也可能延迟，甚至收不到。\n\n这种情况下，开启 VPN 往往会更稳定，因为项目当前使用的主要推送链路就是 FCM。',
      '所以这个提醒功能的正确理解应该是：\n\n它不是完全离线、完全本地、完全不依赖网络的提醒能力。只要你依赖的是 FCM 远程推送，消息送达就会同时受到通知权限、系统策略和网络环境影响。',
      '如果你希望签到提醒更稳定，建议：\n\n开启系统通知权限\n允许 app 在后台活动\n关闭不必要的电池优化\n不要频繁手动清理 app\n如果你所在网络环境对 FCM 不稳定，可以开启 VPN',
    ],
  ),
  const AboutSectionContent(
