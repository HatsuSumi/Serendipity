...
final List<AboutSectionContent> designDecisionSections = List.unmodifiable([
  ...
  const AboutSectionContent(
    title: '为什么签到提醒有时候不准、没触发，或者切回前台才出现？',
    paragraphs: [
      '先说结论：这通常不是 app 内部定时器本身失效，而是手机系统在后台把提醒延后了。',
      '签到提醒依赖的是系统通知能力，而不是要求 app 一直在前台运行。\n\n但不同手机厂商对后台行为的限制非常激进，尤其是省电策略、电池优化、自启动限制、后台冻结、通知权限、锁屏清理等设置，都会影响提醒是否准时触发。',
      '为什么会出现“切回前台才提示”？\n\n常见原因是：app 在后台时被系统挂起了，和提醒相关的后台调度没有按预期执行；等你重新打开 app，系统恢复了进程或 app 重新完成了一次检查，于是提醒才立刻出现。',
      '很多人会遇到一种情况：提醒时间已经过了，但通知一直没出现；等你重新打开 app，提醒才突然弹出来。\n\n这通常不是你设置错了，而是不同手机系统对后台限制的强度不一样。',
      '哪些情况最容易导致提醒不准？\n\n1. 没有授予通知权限\n2. 开了系统电池优化/省电模式\n3. app 不允许后台运行或自启动\n4. 被系统一键清理、锁屏后冻结、长时间待机\n5. 部分 Android 厂商对定时任务和后台进程限制特别严格',
      '所以这个提醒功能的正确理解应该是：\n\n它会尽力按设定时间提醒，但最终是否能在后台、锁屏、待机状态下绝对准时触发，仍然受手机系统策略影响，app 无法百分之百控制。',
      '如果你很在意签到提醒的准时性，建议：\n\n开启系统通知权限\n关闭该 app 的电池优化\n允许后台活动和自启动\n不要把 app 从最近任务里手动划掉\n在对应厂商系统里把它加入白名单',
    ],
  ),
  const AboutSectionContent(
    title: '为什么其他 app 可以在后台也准时推送？',
    paragraphs: [
      '因为很多 app 走的不是同一条路。',
      '有些 app 用的是服务器推送、厂商通道，或者本身就有更高的系统优先级，所以即使不在前台，也更容易准时送达。',
      '而普通 app 的本地提醒，更容易受到省电策略、后台限制和系统清理影响。',
      '总之，大厂和独立开发者是两回事，我一人开发没那个能力，就是个废物。'
    ],
  ),
  const AboutSectionContent(
