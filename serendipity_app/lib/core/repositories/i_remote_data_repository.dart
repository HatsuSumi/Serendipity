...
/// 服务端测试推送结果摘要
class RepositoryServerTestPushSummary {
  final String dispatchSource;
  final int scannedCandidates;
  final int sentCount;
  final int failedCount;

  const RepositoryServerTestPushSummary({
  ...
  factory RepositoryServerTestPushSummary.fromJson(Map<String, dynamic> json) {
    ...
    final dispatchSource = json['dispatchSource'];
    return RepositoryServerTestPushSummary(
      dispatchSource: dispatchSource is String && dispatchSource.isNotEmpty
          ? dispatchSource
          : 'manual_test',
      scannedCandidates: readInt('scannedCandidates'),
      sentCount: readInt('sentCount'),
...
class RepositoryPushTokenRecord {
  final String id;
  final String token;
  final bool isActive;
  ...
  const RepositoryPushTokenRecord({
    required this.id,
    required this.token,
  ...
  factory RepositoryPushTokenRecord.fromJson(Map<String, dynamic> json) {
    ...
    return RepositoryPushTokenRecord(
...
class RepositoryPushTokenStatus {
  final List<RepositoryPushTokenRecord> pushTokens;

  const RepositoryPushTokenStatus({required this.pushTokens});

  factory RepositoryPushTokenStatus.fromJson(Map<String, dynamic> json) {
    final rawList = (json['pushTokens'] as List?) ?? const [];
    return RepositoryPushTokenStatus(
      pushTokens: rawList
          .whereType<Map<String, dynamic>>()
          .map(RepositoryPushTokenRecord.fromJson)
          .where((pushToken) => pushToken.id.isNotEmpty && pushToken.token.isNotEmpty)
          .toList(),
    );
  }
}
